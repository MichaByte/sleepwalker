function startGame() {
  ...
  newLine(
    'Welcome to Sleepwalker! You will get options on what to say or do, and you can choose between them by simply typing the number before each option.<br><br>'
  );
  newLine(
    'You wake up and notice you are lying on the ground and not in bed. You look around your room and notice it\'s very out of order. Some of the large furniture has been knocked over. You realize that you must have been sleepwalking. <br> You decide to investigate further. <br><br>'
  );
  bedroom();
}
...
function bedroom() {
  ...
  if (!rooms.bedroom.clean) {
    newLine(
      'As you walk around your bedroom, you see your lamp is knocked over, and your desk is a mess. You seem to be missing your wallet, as well as your spare house key. <br> What would you like to investigate next? <br><br>',
      "",
    ...
    newLine('It\'s your room. The bed is made, the desk is clean, and everything is in it\'s place. Where do you want to go next? <br><br>', "",
...
function kitchen() {
  ...
  if (!rooms.kitchen.clean) {
    newLine(
      'There are shards of fine china and dented pots strewn across the floor. The silverware drawers are empty, and all your chairs seem to be missing. You notice a bag of flour spilled on the ground, and there are multiple sets of footsteps in the fine white powder. <br> Where would you like to investigate next? <br><br>',
      "",
    ...
    newLine('The kitchen is clean, the dishes stacked, and everything is in its place. Where would you like to go next? <br><br>', "",
      ['Press "1" to check out the bedroom.', 'Press "2" to check out the bathroom.', 'Press "3" to leave the house']
    );
    nextPlaces = ["bedroom", "bathroom", "outside"];
  }
...
function bathroom() {
  if (!rooms.bathroom.clean) {
    newLine(
      'As you walk into the bathroom, you notice that your toiletries are spilled all over the floor, and the paintings that were once on your wall are on the floor. <br>You quickly tidy up and wipe down the surfaces.<br> Where would you like to investigate next? <br><br>',
      "",
    ...
    newLine('Where would you like to go next? <br><br>', "",
      ['Press "1" to check out the bedroom.', 'Press "2" to check out the kitchen.', 'Press "3" to leave the house']
    );
    nextPlaces = ["bedroom", "kitchen", "outside"];
  }
...
function outside() {
  newLine(
    "Since you already investigated all the rooms in your house, you decide to go outside. From here, you can go to the park or stop by and visit your neighbor. <br><br>",
    "",
...
function park() {
  ...
  newLine('<br><br>Press "1" to go back');
  nextPlaces = ["outside"];
}
function oldmanHouse() {
  ...
  if (!rooms.oldman.visited && !rooms.oldman.happy) {
    ...
    newDialog(
      ...
      "oldmanneighbor",
      "You have no right to show your face around here after what you did last night! <br>"
    );
    newLine(
    ...
    let needsSomething = newDialog(
      "assets/img/oldmanneighbor.png",
      "oldmanneighbor",
      "What do you think you're doing here? I'm not talkin' to you 'till you make amends. <br>"
    );
    ...
    newDialog(
      "assets/img/oldmanneighbor.png",
      "oldmanneighbor",
      "Thanks for bringing me that lovely lamp! Oh, by the way, you dropped this while you were out an' about last night. <br>"
    );
    addItemToInventory("key");
    newLine("<br><br>Press \"1\" to go back outside");
    nextPlaces = ["outside"];
  }
}
function questionOldman() {
  let needsSomething = newDialog(
    "assets/img/oldmanneighbor.png",
    "oldmanneighbor",
    "<br>You showed up at my house at three in the morning and threw a chair at my window! Sleepwalking or not, you will need to pay for the damages! <br>"
  );
  newLine(
    'How do you respond?<br><br>',
    "",
...
function angryOldman() {
  newDialog(
    "assets/img/oldmanneighbor.png",
    "oldmanneighbor",
    "Don't be snarky with me, you know damn well you broke my window last night! I'm calling the cops! <br>"
  );
  newLine(
    "He slams the door in your face and refuses to answer any questions you yell at his locked door. <br>"
  );
  newLine(
...
function leaveOldman() {
  let needsSomething = newDialog(
    "assets/img/oldmanneighbor.png",
    "oldmanneighbor",
    "I believe you went over that way afterward, I was tired and not paying very much attention though. You can go check it out, but I expect you to come back and pay me later! <br>"
  );
  newLine("Oh! There's a coin on the floor, you pick it up.");
  addItemToInventory("money");
  newLine(
    '<br>You start walking in the direction he pointed, and as you walk, you feel the area is vaguely familiar, but you cannot put your finger on it. “I must be starting to remember parts of my sleepwalking”, You think to yourself.<br>After some walking, you come across a stream and you see a tree branch that you could use to cross the stream. You also see a bridge in the distance, but it appears to be a few minutes walk away. What would you like to do? <br><br>',
    "",
...
function crossBranch() {
  newLine(
    "<br>As you cross the tree branch, it snaps beneath your weight. You fall into the stream, and the current takes you far from your home. <br>"
  );
  newLine(
...
function crossBridge() {
  ...
  let needsSomething = newDialog("assets/img/femaleneighbor.png", "femaleneighbor", "Who are you, and what do you want? <br>");
  newLine(
    'What would you  like to say: <br><br>',
    "",
...
function femaleNeighbor() {
  ...
  if (!rooms.femaleneighbor.visited && !rooms.femaleneighbor.happy) {
    let needsSomething = newDialog(
      ...
      "femaleneighbor",
      "Someone ripped all the plants out of my garden last night! Do you have any idea who it could have been? <br>"
    );
    addQuest("femaleneighbor", "Find some flowers to replace the ones that were uprooted", "flower", 3);
    newLine(
      'How would you like to respond: <br><br>',
      "",
    ...
    let needsSomething = newDialog(
      "assets/img/femaleneighbor.png",
      "femaleneighbor",
      "My beautiful flowers! All gone! If only someone would pull me out of this utter despair. <br>"
    );
    if (needsSomething) newLine('<br>Use "g &#60;item>" to give items');
  }
  else {
    newDialog(
      "assets/img/femaleneighbor.png",
      "femaleneighbor",
      "Oh, you brought me some flowers! Thank you so much! I'll go plant them immediately. And, by the way, I've got these lovely plates I don't use anymore, you can have them if they tickle you fancy. <br>"
    );
    addItemToInventory("plate");
    addItemToInventory("plate");
    newLine("<br>Press \"1\" to go back outside");
    nextPlaces = ["outside"];
  }
  nextPlaces = ["angryFemaleNeighbor", "leaveFemaleNeighbor"];
}
function angryFemaleNeighbor() {
  newDialog("assets/img/femaleneighbor.png", "femaleneighbor", "WHAT THE HELL! You are going to stay here for the rest of the day, and replant for me, or I will be calling the cops! <br>");
  newLine(
    'You have lost the game.',
    "red",
    ['<br>Press "1" to reset. You will need to run the command "start" again.']
  );
  nextPlaces = ["endGame"];
}
function leaveFemaleNeighbor() {
  newDialog("assets/img/femaleneighbor.png", "femaleneighbor", "Ok, but you better let me know if you find out anything. And it\'d be great if you could get me some plants to replace the ones that were stolen. <br>");
...
function howtoplay() {
  if (document.getElementsByClassName("play")[0]) {
    ...
    newLine(`Welcome to Sleepwalker! Navigate a world of mysteries and challenges to complete your quest and make things right with your neighbors. Here’s how:<br><br>
            Getting Started:
            Run the “start” command to begin the game inside the terminal input.<br><br><br>
            Controls:<br>
            Keyboard: Use digits (e.g. 1, 2, 3) to select options, and press i to view your items.<br>
            Actions: Press [Enter] to confirm choices or interact with objects.<br><br><br>
